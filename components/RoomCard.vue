<template>
  <div :id="room.id" class="room-card" v-on:click="enterRoom($event, room.id, room.meeting_info.date + ' || ' + room.meeting_info.type + ' || ' + room.receiver_info.nickname + '(' + room.receiver_info.age + ')' + ', ' + room.sender_info.nickname + '(' + room.sender_info.age + ')')">
...
export default {
    ...
    props: ["room"],
    methods : {
        enterRoom: function(event, id, title) {
